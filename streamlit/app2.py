...
def show_album_page(album_name):
    ...
    for filename in image_files:
        # Streamlitの静的ファイル機能に対応したURLパス
        # `replace()`を使って、\を/に置き換える
        image_url = os.path.join("static", "albums", album_name, filename).replace("\\", "/")
        
        # <a>タグのURLは元の画像（2-）を参照するように修正
        full_image_url = image_url.replace('/1-', '/2-')
        
        st.markdown(f"""
            <a href="{full_image_url}" target="_blank">
                <img class="grid-item" src="{image_url}" alt="{album_name}">
            </a>
        """, unsafe_allow_html=True)
