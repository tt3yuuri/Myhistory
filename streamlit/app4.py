...
def show_album_page(album_name):
    ...
    # アルバム内の画像ファイルを取得（幅400pxの画像のみ）
    image_files = sorted([f for f in os.listdir(album_path) if f.startswith('1-')])

    # HTMLとCSSを埋め込むことで、グリッドレイアウトを再現
    st.markdown("""
        <style>
            .grid {
                display: grid;
                ...
                grid-template-rows: 46vw 46vw 46vw 46vw;
            }
            .grid-item {
                width: 100%;
                height: 100%;
    ...
    for filename in image_files:
        st.markdown(f"""
            <a href="{os.path.join(album_path, filename)}" target="_blank">
                <img class="grid-item" src="{os.path.join(album_path, filename)}" alt="{album_name}">
            </a>
        """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if not album_names:
            ...
            for i, album_name in enumerate(album_names):
                with cols[i % 4]:
                    if st.button(f"'{album_name}'", key=f"album_btn_{album_name}"):
